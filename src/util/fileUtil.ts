...
export function formatDatetime(date: number) {
  ...
  const localDatetime = new Date(new Date(date).getTime() - tzOffsetMs) // Subtract tz offset from datetime to get local datetime
  return localDatetime.toISOString().slice(0, 19).replace(/-/g, "/").replace("T", " at ") // ISO format and return local datetime
}

export function getIconByMIMEType (fileType: string, isFolder: boolean) {
